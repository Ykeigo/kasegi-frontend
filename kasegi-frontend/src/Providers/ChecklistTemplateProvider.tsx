 ...
 */
export const ChecklistTemplateProvider: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
  // ユーザ情報の初期値
  const [checklistTemplate, setMyTips] = useState<ChecklistTemplate>({id: "hoge", checkItems: ["楽しかった？", "ベストを尽くした？"]});

  const get = () => checklistTemplate

  const set = (entity: ChecklistTemplate) =>
    setMyTips( entity );

  return (
    <ChecklistTemplateContext.Provider
      value={{ checklistTemplate, set, get }}
    >
      {children}
    </ChecklistTemplateContext.Provider>
  );
